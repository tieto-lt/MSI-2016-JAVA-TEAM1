     ...
     function setSelectedObjects(array){
        ...
        for( i = 0; i<vm.all.length; i++){
            for( j = 0; j<array.length; j++){
                if(vm.all[i].name == array[j].name){
                    vm.selected[vm.selected.length] = {name:array[j].name, cameraPosition: array[j].cameraPosition, color: "#009688"};
                }
                else{
                    vm.selected[vm.selected.length] = {name: "empty", cameraPosition: "empty", color: "#928f8f"};
                    }
            }
        }
